...
func TestFilterCandidates(t *testing.T) {
	...
	t.Run("Empty input returns empty", func(t *testing.T) {
		if got := common.FilterCandidates(nil); len(got) != 0 {
	...
	t.Run("All healthy and not draining", func(t *testing.T) {
		...
		if got := common.FilterCandidates(list); len(got) != 2 {
	...
	t.Run("Filters unhealthy", func(t *testing.T) {
		b1.SetHealthy(false)
		b2.SetHealthy(true)
		if got := common.FilterCandidates([]*core.Backend{b1, b2}); len(got) != 1 || got[0] != b2 {
	...
	t.Run("Preserves order", func(t *testing.T) {
		...
		got := common.FilterCandidates(input)
