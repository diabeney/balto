...
export function RequestRateChart({ data, title = 'Request Rate' }: RequestRateChartProps) {
  // Handle empty data
  ...
  // Get unique colors for each series
        ...
        <AreaChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          <defs>
            {data.map((series, index) => (
              <linearGradient key={series.id || `gradient-${index}`} id={`requestRateGradient${index}`} x1="0" y1="0" x2="0" y2="1">
          ...
          <YAxis 
            className="text-xs text-zinc-600 dark:text-zinc-400"
            tick={{ fill: 'currentColor', fontFamily: 'var(--font-mono)', fontSize: 10 }}
            label={{ value: 'Requests', angle: -90, position: 'insideLeft', style: { fontSize: 10, fontFamily: 'var(--font-mono)' } }}
          ...
          {data.map((series, index) => (
            <Area
              ...
              fill={`url(#requestRateGradient${index})`}
              strokeWidth={2}
            />
          ))}
        </AreaChart>
