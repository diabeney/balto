...
export function MemoryUsageChart({ data, title = 'Memory Usage' }: MemoryUsageChartProps) {
  // Handle empty data
  ...
  const chartData = sortedTimes.map(time => {
    ...
    data.forEach(series => {
      ...
      // Convert bytes to MB
      point[series.label] = (matchingPoint?.value || 0) / (1024 * 1024);
    });
    return point;
  });
        ...
        <AreaChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          <defs>
            {data.map((series, index) => (
              <linearGradient key={series.id || `gradient-${index}`} id={`memoryGradient${index}`} x1="0" y1="0" x2="0" y2="1">
          ...
          <YAxis 
            className="text-xs text-zinc-600 dark:text-zinc-400"
            tick={{ fill: 'currentColor', fontFamily: 'var(--font-mono)', fontSize: 10 }}
            label={{ value: 'MB', angle: -90, position: 'insideLeft', style: { fontSize: 10, fontFamily: 'var(--font-mono)' } }}
          />
          <Tooltip 
            contentStyle={{
